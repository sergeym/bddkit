...
- **A plugin instance handle is a `u64`, not a pointer.** It indexes the plugin's own table, so it is `Send + Sync` for free and can never dangle into a use-after-free on the host side — a stale or forged handle is data the plugin answers `{"status":"fatal","error":"unknown handle"}` to. The `Mutex` in `Plugins` protects the map, not the handle. Do not "optimise" it back into an opaque pointer.
...
- **Plugin libraries are deliberately leaked at exit.** `load_plugins` does `std::mem::forget(plugins.clone())` so the mapping outlives every exit path — an early `?`, `process::exit(2)`, the happy path. Running a thread-local destructor or an `atexit` handler after `dlclose` executes an unmapped page; the process is exiting anyway. That leaks the MAPPING only — instances are still dropped through FFI by `Plugins::shutdown` after the pool drains, and `tests/plugin.rs::shutdown_drops_the_instance_even_after_a_failed_run` is what pins that call site.
- **`<<null>>` reaches a plugin as two literal NUL bytes inside a `String`.** `serde_json` escapes them to `\u0000` on the wire, so the host's own `CString::new(request)` is fine — but after the plugin's parse the argument holds real NULs and `CString::new(arg)` returns `NulError`. The fixture's `out` helper shows the fallback. Nothing in the host tests this path; it is the first thing a real plugin will hit.
