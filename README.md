...
| Step | Effect |
|---|---|
| `I include "<file>"` | Runs the file's sole scenario — a Scenario Outline is only includable this way if its Examples table holds exactly one row; more than one row (or none) requires a `with:` table to pick one, and is otherwise a validation error; the included scenario's HTTP/DB/plugin state flows back to the caller, but variables are isolated |
...
A `with:` table feeds a single row of parameters into an included Scenario Outline — the table must have exactly one data row (a header row plus one values row); anything else (zero data rows, or two or more) is a hard validation error, "with: must have exactly one data row." The caller builds the cells by interpolating `<<...>>` values **against the caller's own variables**, not the included file's scope. The included scenario's `@exports(a,b,*)` tag declares which result variables come back to the caller; undeclared variables are discarded when the scenario ends. `@exports(*)` brings back all of them, `@exports(user_id, token)` brings back only those two. For a scenario with no `@exports` tag, nothing returns.

State that carries across: the HTTP response from the last request, the current API resource and its headers, the current database connection, and plugin instances with their per-scenario state (a plugin's instance selection resets like the API does at every scenario boundary). Variables do not carry across — only what the `@exports` tag names comes back; all other variables are discarded.
...
- A call-site export prefix — letting the caller spell `I include "setup" as "u"` and get back `u_user_id` instead of `user_id` — has no step syntax.
